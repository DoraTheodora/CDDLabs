...
*/
  ...
  \param taskOneArrived gives the function the control of the first part of the function
  \param taskTwoArrived gives the function the control when for the second part of the function
...
void taskOne(std::shared_ptr<Semaphore> taskOneArrived,std::shared_ptr<Semaphore>  taskTwoArrived, int delay){
  ...
  \param taskTwoArrived puts the second part of the thread in the waiting queue, waiting for its turn 
  \param delay indicates the time the thread will sleep
  displays a message that is split in to 2 sections to show how a rendezvous works*/
